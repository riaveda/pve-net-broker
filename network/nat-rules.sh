...
SERVICES=(
    ...
    # ── k3s 워커 메시(headscale) — 아래 SERVICES_UDP 의 51820·51822 와 한 묶음이다 ──
    # ⚠️ 51821 을 쓰는 이유: headscale 기본은 8080 인데, 8080 은 ① 사내망에서 가장 먼저 스캔되는
    #   자리이고 ② homey-nexus 안에서 이미 "구현자 backend" 를 뜻하는 숫자다(같은 숫자가 두 뜻이 된다).
    #   터널 입구(51820) 옆으로 옮겨 대장이 한 줄로 읽히게 한다 — "워커가 걸어 들어오는 입구들".
    # ⚠️ 안팎을 **같은 번호**로 둔다. 다르면 headscale 이 워커에게 알려 주는 포트와 실제로 열린 포트가
    #   갈려 직접 경로만 조용히 안 뚫린다(중계로 돌아 느려지는데 원인이 안 보인다).
    # ⚠️⚠️ 대상이 **`.42`(TLS 종단)** 이지 headscale 이 도는 `.43` 이 아니다 — 실측 2026-09-17:
    #   평문으로 두면 **중계(DERP)에 못 붙고 재등록이 500** 으로 깨진다(tailscale 이 loopback 이 아닌
    #   조정자 주소에 평문을 안 쓴다). `.42` 가 TLS 를 벗겨 `.43:51821` 로 넘긴다
    #   (`reverse-proxy/nginx/tls-available/headscale.conf`). ⚠️ 첫 등록만은 평문으로도 통과해
    #   "되는 줄 알았다" 가 되기 쉽다.
    "51821:10.10.10.42:51821" # k3s 워커 메시 컨트롤 채널 + 중계(DERP) — .42 에서 TLS 종단
)
...
SERVICES_UDP=(
    ...
    "51822:10.10.10.43:51822"  # k3s 워커 메시 STUN — 워커끼리 직접 경로를 뚫을 때 서로의 주소를 알아내는 자리.
                               #   막혀도 중계(51821)로 붙는다 — 느려질 뿐 끊기지는 않는다.
    # ⚠️ 컨트롤 노드에 **고정 종점**을 준다. 이것이 없어도 직접 경로는 서지만(실측 2026-09-17),
    #   그것은 컨트롤 노드가 **먼저 걸어 나가** 생긴 NAT 매핑에 기댄 것이라 **우연**이다.
    #   워커가 먼저 거는 순서에서는 성립하지 않는다 → 한 줄로 못 박는다.
    "41641:10.10.10.43:41641"  # k3s 워커 메시 직접 경로 — 컨트롤 노드의 고정 종점
)
