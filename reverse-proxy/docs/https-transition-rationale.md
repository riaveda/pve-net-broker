...
탈출구가 있어 루트 미설치 사용자가 갇히지 않는다.

⚠️⚠️ **표식은 뜻마다 값이 다르다 — 의도와 사고를 섞지 않는다 (정정 2026-08-20).**
초기 구현은 **확인(probe)이 5초를 넘기면 `skip` 을 심었다.** 그런데 `skip` 은 *사람이 거부했다*는 뜻이라
그 상태에서는 확인이 **다시 돌지 않는다** → 우연히 한 번 느렸을 뿐인데 **그 세션 내내 http 에 갇혔다.**
브라우저를 통째로 닫아야 풀렸고 화면엔 원인이 없어, 며칠 간격으로 같은 증상이 반복됐다(실사고).
→ **지연은 `slow`(`max-age=60`)** 로 심는다. 서빙은 skip 과 같되 **60초 뒤 저절로 사라져 스스로 회복**한다.
probe 는 **한 번 재시도**하고 대기는 8초로 둔다(첫 TLS 핸드셰이크는 원래 느리다).
**규칙**: 자동 장치가 사람이 고른 것과 같은 표식을 남기면 사고가 의도처럼 굳는다 — 사고 쪽에는
**스스로 풀리는 수명**을 준다.

⚠️⚠️ **한 창에서 표명된 의도를 전체에 무기한 적용하지 않는다 (정정 2026-08-24 — 위 규칙의 형제 문제).**
`skip` 은 *사람이 눌렀다*는 뜻이라 위 정정의 대상이 아니었는데, **범위가 틀렸다**:
- `ap_tls` 는 `Path=/` 라 **브라우저 프로필 전체**에 걸린다.
- **설치형 앱(PWA)은 브라우저와 같은 쿠키 저장소**를 쓴다 → 탭에서 누른 「설치 없이 계속」이
  **아무 상관 없는 앱 창까지** http 로 끌고 내려간다.
- 앱 창에는 **주소창도 안내 링크도 없다** → 사용자는 원인을 못 보고, 풀 방법도 화면에 없다.
- 게다가 `skip` 은 세션 쿠키였는데 **앱 창이 크롬 프로세스를 살려 둬서** 세션이 며칠씩 이어졌다.
  증상은 "앱만 계속 주의 요함 → 크롬을 통째로 끄면 낫는다" 로, 몇 주 간격으로 재발했다(실사고).
→ **`skip` 에 `max-age=1800`(30분)** 을 준다. 세션 쿠키도 24시간도 안 된다 — 전자는 위 이유로
  사실상 무기한이 되고, 후자는 루트를 이미 설치한 PC 도 하루 갇힌다.
→ 앱 쪽에서도 **http 로 열렸을 때 그 사실과 복구 링크를 화면에 띄운다**(각 앱 소관, §8).
