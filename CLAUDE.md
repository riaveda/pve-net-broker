...
- **⚠️ 신뢰 표식(쿠키 `ap_tls`) 규율** — 유도가 "되다가 안 되는" 증상의 단일 원인이다.
  ...
  · **⚠️⚠️ "주의 요함" 은 **세 가지**다 — 화면 증상으로는 절대 안 갈린다. 판정은 **DevTools → Security 탭** 하나로 한다.**
    ...
    | `Resources — active mixed content`<br>("recently allowed non-secure content") | **브라우저 사이트 권한** — 언젠가 「안전하지 않은 스크립트 로드」를 누른 것이 **프로필에 영구 저장**됨 | 사이트 팝업의 **「권한 재설정」**(또는 사이트 설정 → 안전하지 않은 콘텐츠 → 차단) 후 새로고침 |
