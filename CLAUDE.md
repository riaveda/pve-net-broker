...
- **⚠️ 신뢰 표식(쿠키 `ap_tls`) 규율** — 유도가 "되다가 안 되는" 증상의 단일 원인이다.
  ...
  · **⚠️⚠️ "주의 요함" 은 **세 가지**다 — 화면 증상으로는 절대 안 갈린다. 판정은 **DevTools → Security 탭** 하나로 한다.**
    ...
    | `Resources — active mixed content`<br>("recently allowed non-secure content") | **브라우저 사이트 권한**(안전하지 않은 콘텐츠 허용) | 사람이 켠 것이면 사이트 팝업의 **「권한 재설정」** 후 새로고침.<br>⛔ **「관리자가 허용함」이면 사용자가 못 고친다** — 사내 크롬 정책이다(아래) |

    ⚠️ **셋째는 우리 인프라와 무관하다** — 인증서도 연결도 정상(`valid and trusted` · TLS 1.3)인데,
      **권한이 켜져 있다는 사실만으로** 크롬이 계속 경고한다. 페이지가 실제로 http 자원을 하나도
      안 불러와도 그렇다(서버에서 `grep 'http://'` 해도 0건이다 — 그래서 서버만 보면 영영 못 찾는다).
    ⚠️ **프로필 단위**라 같은 프로필의 **앱 창과 탭이 함께** 영향을 받고, **브라우저를 꺼도 안 사라진다.**
      "끄면 낫는 것 같다"는 첫째(세션 쿠키)의 성질이라, 셋째를 그 쪽으로 오진하기 쉽다.
    ⚠️⚠️ **사내 크롬 정책으로 강제되면 우리 쪽에서 없앨 방법이 없다 (확정 2026-08-24).** 설정 화면에
      **「관리자가 허용함」** 으로 회색 처리돼 있으면 `InsecureContentAllowedForUrls` 정책이 이 호스트를
      허용 목록에 넣은 것이다 → **모든 사내 PC 에서 이 경고가 영구히 뜬다.**
      · 실제 전송은 안전하다(`Certificate valid and trusted` · TLS 1.3) — **표시만** 경고다.
      · 그래서 **"경고 없는 https"** 는 이 정책이 걸린 호스트에서 달성 불가다. tls-setup 의 목표 중
        그 항목은 우리 손을 떠난 것으로 취급한다(전송 암호화·인증서 신뢰는 그대로 유효).
      · 정책 내용 확인: `chrome://policy` 에서 `InsecureContentAllowedForUrls` 검색.
        목록에서 이 호스트를 빼는 것은 **사내 IT 소관**이다.
      · ⛔ 이 증상을 보고 서버·인증서·쿠키를 다시 뒤지지 않는다 — Security 탭이 위 문구를 보이고
        설정이 회색이면 **거기서 끝**이다.
