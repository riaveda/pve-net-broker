...
- 전제:
  ...
  2. (포털 분리 1회 세팅) `.42`에서 `/var/www/reverse-proxy` 심볼릭을 포털 dist 로 repoint:
     ...
     ✅ 2026-09-16 수행·검증 완료 (그전까지 수 주간 구 경로를 가리켜
     "빌드는 성공하는데 화면이 안 바뀜" 상태였다).
     반드시 **빌드 → repoint** 순서로 (dist 없는 곳으로 먼저 걸면 404).
     확인: `readlink -f /var/www/reverse-proxy` · 상세 절차는 포털 레포 CLAUDE.md
...
- **실행 위치와 계정은 코드블록 바로 위 한 줄**에 굵게 적는다 (블록 안에 주석으로 넣지 않는다).
  호스트가 다르면 같은 명령도 다른 결과를 낸다 — PVE=root(sudo 없음) / `.42`=riaveda(sudo 필요).
...
- 경로는 기억으로 쓰지 않고 위 "호스트/계정" 정본에서 확인한 실제 경로로 쓴다.
...
**1. PVE 호스트 (root) — NAT 변경 반영**

```bash
cd /opt/pve-net-broker && git pull && pnbctl nat reload
```
